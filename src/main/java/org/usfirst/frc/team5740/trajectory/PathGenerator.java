...
public class PathGenerator {
  ...
  static Trajectory generateFromTrapezoidalPath(WaypointSequence path, TrajectoryGenerator.Config config) {
    if (path.getNumWaypoints() < 2) {
      return null;
    }
    ...
    // Generate a smooth trajectory over the total distance.
    Trajectory traj = TrajectoryGenerator.generate(config, TrajectoryGenerator.UnevenTrapezoidalStrategy, 0,
  ...
  static Trajectory generateFromSplitPath(WaypointSequence path1, WaypointSequence path2,
    ...
    // Generate a smooth trajectory over the total distance.
    Trajectory traj1 = TrajectoryGenerator.generate(config, TrajectoryGenerator.TrapezoidalStrategy, 0.0,
        path1.getWaypoint(0).theta, total_distance1, 5.0, path1.getWaypoint(0).theta);

    // Generate a smooth trajectory over the total distance.
    Trajectory traj2 = TrajectoryGenerator.generate(config, TrajectoryGenerator.TrapezoidalStrategy, 5.0,
    ...
    for (int i = 0; i < traj1.getNumSegments(); ++i) {
      ...
      while (!found_spline) {
        ...
        if (cur_pos_relative <= spline_lengths1[cur_spline]) {
          double percentage = splines1[cur_spline].getPercentageForDistance(cur_pos_relative);
    ...
    for (int i = 0; i < traj2.getNumSegments(); ++i) {
      ...
      while (!found_spline) {
        ...
        if (cur_pos_relative <= spline_lengths2[cur_spline]) {
          double percentage = splines2[cur_spline].getPercentageForDistance(cur_pos_relative);
